...
exports.ExtensionMethodSchema = zod_1.z.object({
    ...
    allowedOnPath: zod_1.z
        .array(zod_1.z.union([zod_1.z.string(), zod_1.z.instanceof(RegExp)]))
        .optional()
        .refine((paths) => {
        if (!paths)
            return true;
...
class ZontaxParser {
    ...
    validateInput(source) {
        ...
        if (source.length > this.maxInputLength) {
            throw new ZontaxMergeError(`Input length exceeds maximum allowed length of ${this.maxInputLength} characters`);
        }
    ...
    validateComplexStructure(source) {
    ...
    calculateDelimiterDepth(source) {
        let depth = 0;
        let maxDepth = 0;
        for (let i = 0; i < source.length; i++) {
            const char = source[i];
    ...
    analyzeRegExpComplexity(source) {
        let riskScore = 0;
    ...
    buildDefinition(node, path = [], depth = 0) {
        var _a;
        ...
        if (node.type === "CallExpression") {
            ...
            while (current && current.type === "CallExpression") {
                ...
                if (callee.type === "MemberExpression") {
                    ...
                    const args = current.arguments.map((arg) => this.buildDefinition(arg, path, depth + 1));
                    ...
                    if (extension) {
                        ...
                        if ([
                            ...
                            if (enumArg.type === "ArrayExpression") {
                                data.values = enumArg.elements.map((elem) => this.buildDefinition(elem, path, depth + 1));
                            }
                            else {
                                data.values = args[0];
                            }
                            ...
                            if (tupleArg.type === "ArrayExpression") {
                                data.items = tupleArg.elements.map((elem) => this.buildDefinition(elem, path, depth + 1));
                            }
                            else {
                                data.items = args[0];
                            }
                            ...
                            if (unionArg.type === "ArrayExpression") {
                                data.options = unionArg.elements.map((elem) => this.buildDefinition(elem, path, depth + 1));
                            }
                            else {
                                data.options = args[0];
                            }
    ...
    generateSafeValue(node) {
        ...
        switch (node.type) {
            ...
            case "TemplateLiteral": {
                ...
                return node.quasis
                    .map((quasi) => { var _a, _b, _c, _d; return (_d = (_b = (_a = quasi.value) === null || _a === void 0 ? void 0 : _a.cooked) !== null && _b !== void 0 ? _b : (_c = quasi.value) === null || _c === void 0 ? void 0 : _c.raw) !== null && _d !== void 0 ? _d : ""; })
                    .join("");
            }
        ...
        for (let i = 1; i < defs.length; i++) {
            ...
            if (overlay.validations) {
                ...
                for (const key in overlay.validations) {
                    if (base.validations[key] !== undefined &&
    ...
    generateSchemaString(def) {
        ...
        if (def.type === "object") {
            ...
            const itemsStr = Array.isArray(def.items)
                ? def.items
                    .map((item) => this.generateSchemaString(item))
                    .join(", ")
                : "";
            ...
            const optionsStr = Array.isArray(def.options)
                ? def.options
                    .map((option) => this.generateSchemaString(option))
                    .join(", ")
                : "";
    ...
    parse(...sources) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const definitions = yield Promise.all(sources.map((source) => __awaiter(this, void 0, void 0, function* () {
                ...
                const ast = yield this.parseWithTimeout(source);
